...
/**
 * Created by Миша on 23.09.2018.
 */
public class Seidel extends Main {

    public Seidel(double[][] Ustart) {
        int N=Ustart.length-1;
        double xi = (Math.pow(Math.tan(Math.PI * h / 2), 2)) / 2;
        double ro = (1 - xi) / (1 + xi);
        int n = (int) (1 + Math.log(1 / eps) / (4 * xi));
        System.out.println();
        System.out.println("                                                      МЕТОД ЗЕЙДЕЛЯ-НЕКРАСОВА");
        ...
        while ((norm2(U1, U_acc) / norm2(Ustart, U_acc)) > eps) {
            ...
            for (int i = 1; i < U0.length - 1; i++) {
                for (int j = 1; j < U0.length - 1; j++) {
                    num = F[i][j] + 1 / h / h * (p.applyAsDouble(x[i] - h / 2, x[j]) * U1[i - 1][j] +
                            p.applyAsDouble(x[i] + h / 2, x[j]) * U0[i + 1][j] +
                            q.applyAsDouble(x[i], x[j] - h / 2) * U1[i][j - 1] +
                            q.applyAsDouble(x[i], x[j] + h / 2) * U0[i][j + 1]);
