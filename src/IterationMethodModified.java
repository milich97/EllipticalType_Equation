 ...
 */
public class IterationMethodModified extends Main {

    public IterationMethodModified(double[][] Ustart) {
        int N=Ustart.length-1;
        double xi = (Math.pow(Math.tan(Math.PI * h / 2), 2)) / 2;
        double ro = (1 - xi) / (1 + xi);
        int n = (int) (1 + Math.log(1 / eps) / (2 * xi));
        System.out.println();
        System.out.println("                                                      МЕТОД ИТЕРАЦИИ С ОПТИМАЛЬНЫМ ПАРАМЕТРОМ");
        System.out.println("Оценка числа итераций: " + n);
        System.out.println("Спектральный радиус матрицы перехода: " + ro);
        int k = 0;
        double tau = h * h / (4 * (1 + Math.pow(Math.cos(Math.PI * h / 2), 2)));
